...
def get_user(id):
    try:
        ...
        return dict(row) if (row := data_cursor.fetchone()) else None
    finally:
        data_conn.close()
...
def update_quiz_user_id(id, user_id):
    try:
        data_conn = get_db_connection()
        data_cursor = data_conn.cursor()
...
def update_quiz_status(id, status):
    try:
        data_conn = get_db_connection()
        data_cursor = data_conn.cursor()
...
def update_quiz_current_question_id(id, current_question_id):
    try:
        data_conn = get_db_connection()
        data_cursor = data_conn.cursor()
...
def update_quiz_question_start_time(id, question_start_time):
    try:
        data_conn = get_db_connection()
        data_cursor = data_conn.cursor()
...
def get_questions(quiz_id: str) -> list[dict]:
    ...
    """
    try:
        with get_db_connection() as data_conn:
            data_conn.row_factory = sqlite3.Row
            data_cursor = data_conn.cursor()
            data_cursor.execute("SELECT * FROM questions WHERE quiz_id = ?", (quiz_id,))
            return dict(row) if (row := data_cursor.fetchall()) else None
    except sqlite3.Error as e:
        return []
...
def get_quizzes_by_user(user_id):
    try:
        data_conn = get_db_connection(with_row_factory=True)
        data_cursor = data_conn.cursor()
